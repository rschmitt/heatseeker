...
impl Terminal {
    ...
    fn translate_bytes(bytes: Vec<u8>) -> Vec<Key> {
        if bytes == [27, b'[' as u8, b'A' as u8] { return vec![Up] };
        if bytes == [27, b'[' as u8, b'B' as u8] { return vec![Down] };
        if bytes == [27, b'O' as u8, b'A' as u8] { return vec![Up] };
        if bytes == [27, b'O' as u8, b'B' as u8] { return vec![Down] };
